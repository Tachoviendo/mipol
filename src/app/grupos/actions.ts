...
import { agregarComentario, agregarPublicacion, crearGrupo, esMiembroDe } from "@/data/grupos";
import { entregarTarea, obtenerGrupo, obtenerPublicacion } from "@/data/grupos";
import { usuarioSimuladoDeRol } from "@/data/grupos";
...
export async function entregarTareaAction(formData: FormData) {
  ...
  if (!puedeEntregarTareas(rolActual)) {
    throw new Error("No tenés permiso para entregar tareas.");
  }
